...
class Bot(_main_bot):  # type: ignore
    """The fxstcloud bot class. This is a subclass of :class:`discord.Bot` if you use Pycord.
    ...
    **kwargs:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        FxConfig.lang = language
        FxConfig.default_lang = default_language
    ...
    async def get_application_context(self, interaction: discord.Interaction, cls=FxContext):
    ...
    def add_help_command(
        ...
        **kwargs:
        ...
        if not DPY:
            self.load_extension("fxstcloud.cogs.pyc.help_setup", package="fxstcloud")
    ...
    def add_status_changer(
        ...
        **kwargs:
        ...
        if not DPY:
            self.load_extension("fxstcloud.cogs.pyc.status_changer_setup", package="fxstcloud")
    ...
    def add_blacklist(
        ...
        *,
        ...
        disabled_commands: list[FxConfig.BLACKLIST_COMMANDS] | None = None,
        **kwargs: Callable,
    ):
        ...
        **kwargs:
        ...
        for name, func in kwargs.items():
            if name not in FxConfig.BLACKLIST_COMMANDS.__args__:  # type: ignore
                raise ValueError(
                    f"Invalid blacklist command name '{name}'. "
                    f"Possible values are: {FxConfig.BLACKLIST_COMMANDS.__args__}."  # type: ignore
                )

            if not asyncio.iscoroutinefunction(func):
                raise TypeError(f"Blacklist command overwrite `{name}` must be async.")

        FxConfig.blacklist = Blacklist(
            db_path,
            db_name,
            raise_error,
            owner_only,
        ...
        FxConfig.admin_guilds = admin_server_ids

        self.enabled_extensions.append("blacklist")
        if not DPY:
            self.load_extension("fxstcloud.cogs.pyc.blacklist_setup", package="fxstcloud")
    ...
    async def setup_hook(self):
        ...
        for ext in self.enabled_extensions:
            await self.load_extension(f".cogs.dpy.{ext}_setup", package="fxstcloud")
...
class PrefixBot(Bot, commands.Bot):
    ...
    This class can be used if you want to use fxstcloud with prefix commands.
...
class BridgeBot(Bot, bridge.Bot):
    ...
    This class can be used if you want to use fxstcloud with bridge commands.
    This is only needed for Pycord.
    """
